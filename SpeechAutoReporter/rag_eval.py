...
import json
# import re
from tools.VectorDB import VectorDBManager # 假设 VectorDB.py 在同一目录下
from tqdm import tqdm #用来显示进度条的库
import os#系统相关的库
from typing import List, Dict, Tuple, Any, Set # 变量类型提示

# --- 配置参数 ---
SAMPLES_FILE_PATH = 'C:\\Users\\94373\\Desktop\\RAG\\SpeechAutoReporter\\templates\\超声报告模板\\sample2.jsonl'  # 您的样本文件路径
TEMPLATES_FILE_PATH = 'C:\\Users\\94373\\Desktop\\RAG\\SpeechAutoReporter\\templates\\超声报告模板\\超声报告模板.jsonl'  # 您的模板知识库文件路径
COLLECTION_NAME = 'disease_templates'  # RAG 数据库中的集合名称
CHROMA_DB_PATH = "C:\\Users\\94373\\Desktop\\RAG\\chroma_db" # RAG 数据库路径 (可以自定义)
EMBED_MODEL_NAME = 'bge-m3' # 使用的词嵌入模型
...
def load_jsonl_data(file_path: str) -> list:
    ...
    data = []
    try:
        with open(file_path, 'r', encoding='utf-8') as f: # 打开一个jsonl文件，json和jsonl的区别在于，json文件是一个单独的json数据，而jsonl每一行是一个单独的json数据
            for line in f:
                print(line)
                data.append(json.loads(line))# 保存数据
...
def preprocess_template_data(templates_input: list) -> Tuple[list, list]:
    """
    预处理模板数据，这个是针对templates\超声报告模板\超声报告模板.jsonl类似文件的，这些病例模板中保存的json键是疾病名称，值是疾病描述。
    在llamaindex中实现的rag有两个概念：
    doc_id和documents，前者记录的是检索信息的唯一标识(名称)，后者记录的是检索信息具体内容(疾病描述)
    返回: (template_doc_ids, template_documents)，两个一一对应的list数据
    """
...
def setup_rag_collection(db_manager: VectorDBManager, # rag中的知识向量库管理器
                         collection_name: str, # 知识向量库中的知识集合名称
                         doc_ids: list, # 文档id
                         documents: list, # 文档内容
                         embed_model: str): # 构建知识向量库使用的嵌入模型
    """初始化或更新 RAG 集合中的模板数据"""
    if not doc_ids:
...
def evaluate_ground_truth_set_retrieval(
    ...
    success_counts = {n: 0 for n in sorted_top_n_values}#记录
