...
file_name = '超声报告模板.txt' # word转txt后的病例模板文件名称
output_name = '超声报告模板_new.jsonl' # 形成正则表达式后输出的文件
...
#process_and_save_to_jsonl(input_path, os.path.join(current_dir, 'temp.jsonl')) # 把txt文件变成按疾病分条的jsonl文件
#generate_regex_jsonl(os.path.join(current_dir, 'temp.jsonl'), output_path) # 为jsonl文件中每一条内容生成正则样本
#clean_template_jsonl(output_path, os.path.join(current_dir, 'new.jsonl')) # 过滤正则样本
#generate_samples_jsonl('/root/autodl-tmp/RAG/SpeechAutoReporter/templates/超声报告模板/超声报告模板.jsonl', os.path.join(current_dir, 'sample2.jsonl')) # 为jsonl文件中每一条正则生成模拟的病例样本
